 ...
 * @author Will Vasquez
 */
public class FrmNuevoVIP extends javax.swing.JDialog {

    /**
     * Creates new form FrmNuevoVIP
     * @param parent
     * @param modal
     */
    public FrmNuevoVIP(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
        setLocationRelativeTo(null);
        setTitle("Vehiculos VIP");
        ((JPanel)getContentPane()).setOpaque(false); 
        ImageIcon uno=new ImageIcon(this.getClass().getResource("/Diseño/vip2.jpg")); 
        ...
        Tabla.getSelectionModel().addListSelectionListener(new ListSelectionListener(){
            ...
            public void valueChanged(ListSelectionEvent event) {
                try {
                    ...
                    String Ubicacion = (new File("ImagenesVIP")).getAbsolutePath() + "\\" + Tabla.getValueAt(Tabla.getSelectedRow(), 8);
                    ...
                    try {
                        ...
                        try {
                            ...
                            Ubicacion = (new File("ImagenesVIP")).getAbsolutePath() + "\\error.png";
                            Imagen = ImageIO.read(new File(Ubicacion));
                            Image ImagenRedimensionada;
                            ImagenRedimensionada = Imagen.getScaledInstance(lblPicture.getWidth(), lblPicture.getHeight(), Image.SCALE_SMOOTH);
                            ImageIcon ImagenFinal = new ImageIcon(ImagenRedimensionada);  
    ...
    private void BtnAgregarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnAgregarActionPerformed
        if (txtmarca.getText().trim().equals("") || txtmodelo.getText().trim().equals("") || txtaño.getText().trim().equals("") || txtprecioalquiler.getText().trim().equals("") || txtpreciovehiculo.getText().trim().equals("")) {
            ...
            try {
                String url = "\\no_disponible.png";
                PreparedStatement st = cn.prepareStatement("SELECT placa FROM vip WHERE placa = ?");
                ...
                if(rs.next()){
                    ...
                    st = cn.prepareStatement("INSERT INTO vip VALUES(?, ?, ?, ?, ?, ?, ?, ?, 0)");
                    ...
                    MostrarDatos();
                    st = cn.prepareStatement("SELECT MAX(codigo_vip) FROM vip");
                    ResultSet rs3 = st.executeQuery();
                    rs3.next();
                    txtcodigo.setText(rs3.getString(1));
                    if (lblPicture.getIcon() != null){
                        String destino = (new File("ImagenesVIP")).getAbsolutePath() + "\\" + txtcodigo.getText() + ".png";
                        ...
                        st = cn.prepareStatement("UPDATE vip SET foto = ? WHERE codigo_vip = ?");
                        st.setString(1, url);
    ...
    private void BtnEliminarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnEliminarActionPerformed
        if(txtcodigo.getText().isEmpty()){
            ...
            try {
                PreparedStatement st = cn.prepareStatement("DELETE FROM vip WHERE codigo_vip = ?");
                ...
                if(eliminar == 0){
                    st.execute();
                    String UbicacionImagen = (new File("ImagenesVIP")).getAbsolutePath() + "\\" + txtcodigo.getText() + ".png";
    ...
    private void BtnModificarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnModificarActionPerformed
        if (txtmarca.getText().trim().equals("") || txtmodelo.getText().trim().equals("") || txtaño.getText().trim().equals("") || txtprecioalquiler.getText().trim().equals("") || txtpreciovehiculo.getText().trim().equals("")) {
            ...
            try {
                ...
                PreparedStatement st = cn.prepareStatement("UPDATE vip SET placa = ?, marca = ?, modelo = ?, año = ?, precio_vehiculo = ?, precio_alquiler = ?, foto = ? WHERE codigo_vip = ?");
                st.setString(1, txtmatricula.getText());
    ...
    void MostrarDatos()
    {
        Limpiar();
        try{
        PreparedStatement st = cn.prepareStatement("SELECT codigo_vip, tipo, placa, marca, modelo, año, CONVERT(decimal(7,2), precio_vehiculo, 0), CONVERT(decimal(7,2), precio_alquiler, 0), foto FROM vip");
        ResultSet rs = st.executeQuery();
        rs.next();
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(FrmNuevoVIP.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                FrmNuevoVIP dialog = new FrmNuevoVIP(new javax.swing.JFrame(), true);
